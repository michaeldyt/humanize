---
name: humanize-rlcr
description: Start RLCR on Codex with Claude as the independent reviewer using the native Stop hook.
type: flow
...
Use this flow as the Codex entrypoint for RLCR. Codex is fixed to
`gpt-5.6-sol:xhigh`; independent reviews use Claude `claude-opus-5:max`.
...
- hard blocking on Claude review failure or empty output
...
3. Never run ad-hoc reviewer commands in place of the hook-managed phase transitions.
...
| `--codex-model MODEL:EFFORT` | Legacy option; Codex-native mode remains fixed | gpt-5.6-sol:xhigh |
| `--codex-timeout SECONDS` | Claude review timeout | 5400 |
...
Both review phases run with Claude `claude-opus-5:max` in safe, read-only mode.
